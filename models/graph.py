...
class Graph:
    def __init__(self, edges, edge_feats, k_neighbors, size, neighbor_idx):
        """
        ...
        ----------
        edges : torch.Tensor
            Contains list with nearest neighbor indices. [b * n * neighbors]. Flat all points in one batch
        edge_feats : torch.Tensor
            Contains edge features: relative point coordinates. [b * n * neighbors, c]. Relative distance between
            neighbor points and center point.
        k_neighbors : int
            Number of nearest neighbors.
        size : tuple(int, int)
            Number of points. [b * n, b * n]
    ...
    def construct_graph(pcloud, nb_neighbors):
        ...
        neighbor_idx = torch.argsort(distance_matrix, -1)
        neighbors = neighbor_idx[..., :nb_neighbors]
